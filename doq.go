...
const (
	doqErrorNoError          = quic.ApplicationErrorCode(0x0)
	doqErrorInternal         = quic.ApplicationErrorCode(0x1)
	doqErrorProtocol         = quic.ApplicationErrorCode(0x2)
	doqErrorRequestCancelled = quic.ApplicationErrorCode(0x3)
)

// DoQUpstream resolves queries using DNS over QUIC (RFC 9250).
//
// One QUIC connection is reused for all queries and every query is sent on its
// own bidirectional stream, as required by RFC 9250, section 4.2.
...
// newDoQUpstream builds an upstream for a quic:// URL. The host name is used for
// certificate validation, unless the URL carries an explicit one:
// quic://1.1.1.1:853?sni=cloudflare-dns.com
func newDoQUpstream(u *url.URL) (*DoQUpstream, error) {
	host := u.Hostname()
	if host == "" {
		return nil, fmt.Errorf("invalid DoQ upstream %q: missing host", u.String())
	}
...
func (d *DoQUpstream) String() string {
	return d.url
}
...
func (d *DoQUpstream) Exchange(ctx context.Context, msg *dns.Msg) (*dns.Msg, error) {
	origID := msg.Id
	msg.Id = doqMessageID
	packed, err := msg.Pack()
	msg.Id = origID
	...
	if err != nil && ctx.Err() == nil {
		// The cached connection may have timed out or been closed by the
		// server: retry once on a fresh connection (RFC 9250, section 5.2).
		slog.Debug("DoQ query failed, retrying on a new connection", "upstream", d.url, "error", err)
		resp, err = d.query(ctx, packed)
	}
	if err != nil {
		return nil, err
...
func (d *DoQUpstream) query(ctx context.Context, packed []byte) (*dns.Msg, error) {
	...
	if err != nil && conn.Context().Err() != nil {
		// The connection is gone, make sure the retry dials a new one.
		d.discard(conn)
	}
	return resp, err
}
...
func (d *DoQUpstream) connection(ctx context.Context) (*quic.Conn, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	...
	d.conn = conn
	slog.Debug("DoQ connection established", "upstream", d.url, "addr", d.addr, "sni", d.serverName)
	return conn, nil
}

// discard drops conn if it is still the cached one and closes it.
func (d *DoQUpstream) discard(conn *quic.Conn) {
	d.mu.Lock()
	if d.conn == conn {
		d.conn = nil
	}
